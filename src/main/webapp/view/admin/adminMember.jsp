...
<body>
	...
	<form action="${path}/admin/adminMemStatusUpdate.do" method="post">
			...
			<c:forEach items="${memberDTOs}" var="member">
				<tr>
					<c:if test="${member.mem_type!= '관리자'}">
						<td>${member.mem_seq}<input type="hidden" name="mem_seq"
							value="${member.mem_seq}"></td>
						<td>${member.mem_id}</td>
						<td><input type="text" name="mem_nick"
		...
		<br> <br> <input type="button" id="btn1" value="뒤로가기"
			onclick="location.href='${path}/';"> <input
			class="btn btn-primary" type="submit" value="입력하기">
